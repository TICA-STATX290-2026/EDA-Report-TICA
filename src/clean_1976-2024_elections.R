...
library(pacman)
library(skimr)
pacman::p_load(tidyverse,ggplot2,dplyr,tidyr)
 
# Load raw data
X1976_2024_president <- read_csv("1976-2024-president.csv")
X1976_2024_president
 
# Quick structure/summary checks before cleaning
X1976_2024_president %>% skim()
summary(X1976_2024_president)
unique(X1976_2024_president$party_simplified)
...
X1976_2024_president_clean <- X1976_2024_president %>%
  ...
  mutate(version = case_when(
    str_detect(version, "^\\d{8}$") ~ version,
    str_detect(version, "^\\d{2}/\\d{2}/\\d{4}$") ~
      format(as.Date(version, format = "%d/%m/%Y"), "%Y%m%d"),
    TRUE ~ NA_character_
  )) %>%
  ...
  # Trim stray leading/trailing whitespace from candidate names
  ...
  # (prevents mismatches like "Ohio" vs "OHIO" when merging with other datasets)
  mutate(state = str_to_upper(str_trim(state)),
         state_po = str_to_upper(str_trim(state_po)))
